...
public class Heap {
    ...
    public void insert(int value) {
        ...
        items[size++] = value; // store item in next available slot and then increment the size!

        bubbleUp(); // if new item is greater than parent, we need to bubble up that item!
    }
    ...
    public int remove() {
        ...
        var root = items[0]; // fancy 1/2 i guess
        items[0] = items[--size]; // make the value of last node to root node!

        // if item (root) < children, bubble down!
        bubbleDown();
        return root; // fancy 2/2 i guess
    }
    ...
    private int largerChildIndex(int index) {
        if (!hasLeftChild(index)) return index; // we fill nodes from left to right

        // if no right child, then left child is the greater one
